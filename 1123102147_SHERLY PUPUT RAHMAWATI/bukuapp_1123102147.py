...
def get_db_connection():
    ...
    return pymysql.connect(
        host='localhost',
        user='root',
        password='sherly',  # Pastikan password sesuai dengan pengaturan MySQL Anda
...
def edit(kode_buku):
    ...
    if request.method == 'POST':
        ...
        try:
            cursor.execute("UPDATE buku_1123102147 SET Nama_Buku=%s, Penerbit=%s, Pengarang=%s, Jumlah_Buku=%s WHERE Kode_Buku=%s", 
            ...
            connection.rollback()
            connection.close()
            return render_template('edit_1123102147.html', book=buku, error="Error updating book: " + str(err))
